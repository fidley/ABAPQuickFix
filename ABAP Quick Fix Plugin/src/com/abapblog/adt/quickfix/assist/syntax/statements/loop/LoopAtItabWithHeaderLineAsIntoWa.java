...
public class LoopAtItabWithHeaderLineAsIntoWa extends StatementAssist {
	...
	private static final String replacePattern = "$1 FROM WA $2";

	public LoopAtItabWithHeaderLineAsIntoWa() {
		super();
	...
	public String getChangedCode() {
		...
		updatedCode = updatedCode.replaceAll(internalTableName + "-", "wa_" + internalTableName + "-");
		updatedCode = updatedCode.replaceFirst(internalTableName,
				internalTableName + " INTO DATA(wa_" + internalTableName + ")");
		if (matchPattern(modifyPattern.replaceFirst("ITAB", internalTableName), updatedCode)) {
			updatedCode = replacePattern(modifyPattern.replaceFirst("ITAB", internalTableName),
					replacePattern.replaceFirst("WA", "wa_" + internalTableName), updatedCode);
		}
		if (matchPattern(deletePattern.replaceFirst("ITAB", internalTableName), updatedCode)) {
			updatedCode = replacePattern(deletePattern.replaceFirst("ITAB", internalTableName),
					replacePattern.replaceFirst("WA", "wa_" + internalTableName), updatedCode);
		}
		return updatedCode;
	}
	...
	public String getAssistShortText() {
		return "Replace Loop At ITAB with Header Line with Loop at Into WA";
	}

	@Override
	public String getAssistLongText() {
		return "Replaces the old fashion LOOP AT ITAB. into LOOP AT ITAB INTO DATA, and replaces all "
				+ " occurences of the ITAB in the loop with work area variable.\n MODIFY and DELETE statemens are also adapted.";
	}
